...
void
serialize_string_1( GByteArray* b, char* s )
{
	g_byte_array_append(b, (unsigned char*) s, strlen(s) + 1);
}

char*
unserialize_string_1( GByteArray* b, int* offset )
{
	GString* s;
	char* r;
	char c;
...
bswabe_pub_serialize( bswabe_pub_t* pub )
{
	...
	int i;     //,j,temp;
	b = g_byte_array_new();
	serialize_string_1(b,  pub->pairing_desc);
...
bswabe_pub_unserialize( GByteArray* b, int free )
{
	...
	offset = 0;
	pub->pairing_desc = unserialize_string_1(b, &offset);
