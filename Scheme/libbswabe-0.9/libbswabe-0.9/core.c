...
bswabe_cph_t*
bswabe_enc( bswabe_pub_t* pub,element_t m, int attrib[]) 
{
 ...
 //    mpz_t p_1;
  ...
  element_set_mpz(m, M);
	...
	*/
	...
	mpz_t final_ans;
	mpz_init(final_ans);
        mpz_mul_ui(final_ans, sigma, 10000);
...
int bswabe_dec( bswabe_pub_t* pub,bswabe_prv_t* prv, bswabe_cph_t* cph,element_t m, int userid)// attrib is P
{
  ...
  element_set_mpz(m, M_d);
	...
	mpz_t final_ans;
	mpz_init(final_ans);	
	mpz_mul_ui(final_ans, sigma_d_m, 10000);
	mpz_add(final_ans, final_ans, M_d);
